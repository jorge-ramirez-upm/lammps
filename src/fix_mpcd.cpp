...
enum { BIG_MOVE, MPCD_MOVE, MPCD_ROTATE };
...
//#define MPCD_DEBUG 1
//#define MPCD_DEBUG_ATOMID 58
//#define MPCD_DEBUG_TIMESTEP 449

/* ---------------------------------------------------------------------- */

FixMPCD::FixMPCD(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg),
...
binmpcd(NULL), stencil(NULL), binmpcdv(NULL), binheadv(NULL), binnextv(NULL)
{

    if (narg < 9) error->all(FLERR, "Illegal fix mpcd command");
    ...
    virtualexist = 1;
    if (strcmp(arg[5], "NULL") == 0) virtualexist = 0;
    else virtualgroup = group->find(arg[5]);

    temperature_mpcd = force->numeric(FLERR, arg[6]);
    gridmpcd = force->numeric(FLERR, arg[7]);
    ...
    if (dimension==2)
        ...
        error->all(FLERR, "fix mpcd not implemented for that dimension");
    gamma = 0.0;

    int iarg = 9;
    while (iarg < narg) {
        if (strcmp(arg[iarg], "inside") == 0) {
            if (iarg + 2 > narg) error->all(FLERR, "Illegal fix mpcd command");
            ...
            else error->all(FLERR, "Illegal fix mpcd command");
            iarg += 2;
        }
            ...
            if (iarg + 2 > narg) error->all(FLERR, "Illegal fix mpcd command");
            if (strcmp(arg[iarg + 1], "yes") == 0) angularflag = 1;
            else if (strcmp(arg[iarg + 1], "no") == 0) angularflag = 0;
            else error->all(FLERR, "Illegal fix mpcd command");
            iarg += 2;
        }
        ...
        else if (strcmp(arg[iarg], "shell") == 0) {
            if (iarg + 2 > narg) error->all(FLERR, "Illegal fix mpcd command");
            shell = force->numeric(FLERR, arg[iarg + 1]);
            iarg += 2;
        }
        else if (strcmp(arg[iarg], "gamma") == 0) {
            if (iarg + 2 > narg) error->all(FLERR, "Illegal fix mpcd command");
            gamma = force->numeric(FLERR, arg[iarg + 1]);
            iarg += 2;
        }
        else error->all(FLERR, "Illegal fix mpcd command");
    }
    ...
    if (nevery <= 0) error->all(FLERR, "Illegal fix mpcd command");
    if (bigexist && biggroup < 0)
        error->all(FLERR, "Could not find fix mpcd group ID");
    if (virtualexist && virtualgroup < 0)
        error->all(FLERR, "Could not find fix mpcd group ID");
    if (virtualexist && gamma <= 0)
        error->all(FLERR, "MPCD needs density of virtual particles");
    if (gridmpcd <= 0.0) error->all(FLERR, "Illegal fix mpcd command");
    if (temperature_mpcd <= 0.0) error->all(FLERR, "Illegal fix mpcd command");
    if (seed <= 0) error->all(FLERR, "Illegal fix mpcd command");
    if (radfactor <= 0.0) error->all(FLERR, "Illegal fix mpcd command");
    if (maxbounceallow < 0) error->all(FLERR, "Illegal fix mpcd command");
    if (cubictol < 0.0 || cubictol > 1.0)
        error->all(FLERR, "Illegal fix mpcd command");
    if (shell < 0.0)
        error->all(FLERR, "Illegal fix mpcd command");
    ...
    randomshift = new RanPark(lmp, seed);

    // initialize data structs and flags

    if (bigexist) biggroupbit = group->bitmask[biggroup];
    else biggroupbit = 0;
    if (virtualexist) virtualgroupbit = group->bitmask[virtualgroup];
    else virtualgroupbit = 0;
...
FixMPCD::~FixMPCD()
{
    ...
    memory->destroy(binheadv);
    memory->destroy(binnext);
    memory->destroy(binnextv);
    ...
    memory->destroy(binmpcd);
    memory->destroy(binmpcdv);
...
void FixMPCD::init()
{
    ...
    if (force->newton_pair == 0)
        error->all(FLERR, "Fix mpcd requires newton pair on");
    if (bigexist && comm->ghost_velocity == 0)
        error->all(FLERR, "Fix mpcd requires ghost atoms store velocity");
    if (bigexist && !atom->torque_flag)
        error->all(FLERR, "Fix mpcd requires atom attribute torque");
    if (initflag && update->dt != dt_big)
        error->all(FLERR, "Cannot change timestep once fix mpcd is setup");
    if (comm->style != 0)
        error->universe_all(FLERR, "Fix mpcd can only currently be used with "
            "comm_style brick");
    ...
    // wallexist = 1 if MPCD wall(s) are defined

    wallexist = 0;
    for (int m = 0; m < modify->nfix; m++) {
        if (strcmp(modify->fix[m]->style, "wall/mpcd") == 0) {
            if (wallexist) error->all(FLERR, "Cannot use fix wall/mpcd more than once");
            wallexist = 1;
            wallfix = (FixWallMPCD *)modify->fix[m];
            nwall = wallfix->nwall;
            ...
            if (wallfix->overlap && overlap == 0 && me == 0)
                error->warning(FLERR,
                    "Fix MPCD walls overlap but fix mpcd overlap not set");
        }
    }
    ...
    for (int i = 0; i < modify->nfix; i++) {
        ...
        if (strcmp(modify->fix[i]->style, "deform") == 0) {
            ...
            if (deform->box_change_shape && deform->remapflag != V_REMAP)
                error->all(FLERR, "Using fix mpcd with inconsistent "
    ...
    // limit initial MPCD velocities if necessary
    ...
    if (me == 0) {
        if (screen)
            fprintf(screen, "  # of rescaled MPCD velocities = %d\n", all);
        if (logfile)
            fprintf(logfile, "  # of rescaled MPCD velocities = %d\n", all);
    }
...
void FixMPCD::setup(int vflag)
{
    setup_bounds();

    if (dist_mpcd_reneigh < nevery*dt_big*vmax && me == 0)
        error->warning(FLERR,
            "Fix mpcd MPCD moves may trigger frequent reneighboring");
    ...
    // perform first bining of MPCD and big particles and walls
    // set reneighflag to turn off MPCD rotation
    // don't do MPCD rotation in setup, only during timestepping
...
/* ----------------------------------------------------------------------
   assign MPCD particles to bins
   assign big particles to all bins they overlap
------------------------------------------------------------------------- */

void FixMPCD::pre_neighbor()
{
    int i, j, m, ix, iy, iz, jx, jy, jz, ibin, jbin, lo, hi;
    double rsq, cutbinsq;

    // grow MPCD per-atom bin arrays if necessary
    ...
    if (atom->nmax > nmax) {
        ...
        memory->destroy(binmpcdv);
        memory->destroy(binnextv);
        memory->create(binmpcd, nmax, "fix/mpcd:binmpcd");
        memory->create(binnext, nmax, "fix/mpcd:binnext");
        memory->create(binmpcdv, nmax, "fix/mpcd:binmpcdv");
        memory->create(binnextv, nmax, "fix/mpcd:binnextv");
    }
    ...
    if (bigexist || wallexist) {
        ...
        if (ninfo > maxbig) {
            ...
            biglist = (Big *)memory->smalloc(maxbig * sizeof(Big), "fix/mpcd:biglist");
        }
        ...
        while (i < nall) {
            if (mask[i] & biggroupbit) {
                ...
                if (ix < 0 || ix >= nbin2x || iy < 0 || iy >= nbin2y ||
                    iz < 0 || iz >= nbin2z)
                    error->one(FLERR, "Fix MPCD: bad search bin assignment");
                ...
                for (j = 0; j < nstencil; j++) {
                    ...
                    if (jx < 0 || jx >= nbin2x || jy < 0 || jy >= nbin2y ||
                        ...
                        error->one(FLERR, "Fix MPCD: bad stencil bin for big particle");
                    }
                    rsq = point_bin_distance(x[i], jx, jy, jz);
                    if (rsq < cutbinsq) {
                        jbin = ibin + stencil[j][3];
                        if (nbinbig[jbin] == ATOMPERBIN)
                            error->one(FLERR, "Fix MPCD: too many big particles in bin");
            ...
            if (dim == 0) {
                if (side == 0) {
                    ...
                    if (hi >= nbin2x) error->all(FLERR,
                        "Fix MPCD: bad search bin assignment");
                    lo = 0;
                }
                else {
                    lo = static_cast<int> ((xwall[m] - delta - xblo2)*bininv2x);
                    if (lo >= nbin2x) continue;
                    if (lo < 0) error->all(FLERR, "Fix MPCD: bad search bin assignment");
                    hi = nbin2x - 1;
                }
                    ...
                    for (iy = 0; iy < nbin2y; iy++)
                        for (iz = 0; iz < nbin2z; iz++) {
                            ...
                            if (nbinbig[ibin] == ATOMPERBIN)
                                error->all(FLERR, "Fix MPCD: too many walls in bin");
    ...
    // rotate MPCD velocities on MPCD timestep
    // done now since all MPCDs are currently inside my sub-domain

    if (reneighflag == MPCD_ROTATE) reset_velocities();

    // log stats if reneighboring occurred b/c MPCDs moved too far

    if (reneighflag == MPCD_MOVE) reneighcount++;
    reneighflag = BIG_MOVE;
}

/* ----------------------------------------------------------------------
   advect MPCD particles and detect collisions between MPCD and BIG particles
   when collision occurs, change x,v of MPCD, force,torque of BIG particle
------------------------------------------------------------------------- */

void FixMPCD::post_force(int vflag)
{
    int i, m, ix, iy, iz;
    ...
    if (bigexist || wallexist) {
        for (i = 0; i < nlocal; i++)
            if (mask[i] & groupbit) {
                ...
                iy = static_cast<int> ((x[i][1] - yblo2)*bininv2y);
                iz = static_cast<int> ((x[i][2] - zblo2)*bininv2z);
                binmpcd[i] = iz * nbin2y*nbin2x + iy * nbin2x + ix;

                if (ix < 0 || ix >= nbin2x || iy < 0 || iy >= nbin2y ||
                    iz < 0 || iz >= nbin2z) {
                    if (screen) {
                        fprintf(screen, "MPCD particle " TAGINT_FORMAT
                    ...
                    error->one(FLERR, "Fix MPCD: bad bin assignment for MPCD advection");
                }
            }
    ...
    // detect collision of MPCDs with BIG particles or walls
    ...
    // if any MPCD particle has moved too far, trigger reneigh on next step
    ...
    for (i = 0; i < nlocal; i++)
        if (mask[i] & groupbit) {
            if (x[i][0] < mpcdlo_reneigh[0] || x[i][0] > mpcdhi_reneigh[0] ||
                x[i][1] < mpcdlo_reneigh[1] || x[i][1] > mpcdhi_reneigh[1] ||
                x[i][2] < mpcdlo_reneigh[2] || x[i][2] > mpcdhi_reneigh[2]) flag = 1;
        }
    ...
    // if next timestep is MPCD timestep, trigger reneigh

    if ((update->ntimestep + 1) % nevery == 0) {
        next_reneighbor = update->ntimestep + 1;
        reneighflag = MPCD_ROTATE;
    }
}

/* ----------------------------------------------------------------------
   set coordinates and velocities of virtual MPCD particles
------------------------------------------------------------------------- */
void FixMPCD::set_virtual_particles() {
    int *mask = atom->mask;
    double **x = atom->x;
    double **v = atom->v;
    ...
    // THE FOLLOWING DOES NOT WORK IN PARALLEL
    bigint nbig = group->count(biggroup);
    bigint nvirtual = group->count(virtualgroup);

    // Count local big particles
    int bigcount = 0;
    int virtualcount = 0;
    double R;
    for (int i = 0; i < nlocal; i++) {
    ...
    int NV = M_PI*shell*bigcount*(2 * R - shell) * 10; // JR: CHECK GAMMA


}

/* ----------------------------------------------------------------------
   reset MPCD velocities
   may perform random shifting by up to 1/2 bin in each dimension
   called at pre-neighbor stage when all MPCDs are now inside my sub-domain
------------------------------------------------------------------------- */

void FixMPCD::reset_velocities()
{
    int i, j, n, ix, iy, iz, ibin;
    double vsum[3], u[3], rsum[3], Isum[6], Iinv[6], DL[3], wL[3];
    double vsq;
    double *vave, *rcm;
    double vx, vy, vz;

    // Perform a dynamic shift of bin positions
    ...
    double *corner = shifts[1].corner;
    int *binlo = shifts[1].binlo;
    int *binhi = shifts[1].binhi;
    int nbins = shifts[1].nbins;
    int nbinx = shifts[1].nbinx;
    int nbiny = shifts[1].nbiny;
    BinAve *vbin = shifts[1].vbin;

    // binhead = 1st MPCD particle in each bin
    ...
    // PLACE ALL VIRTUAL PARTICLES AND ASSIGN VELOCITIES
    if (bigexist && virtualexist)
        set_virtual_particles();

    for (i = 0; i < nlocal; i++)
        if ((mask[i] & groupbit) || (mask[i] & virtualgroupbit)) {
    ...
    // JORGE: Calculate average velocities in each cell
    ...
    // If angularflag: 1) Calculate center of mass of particles in cell
    // and add appropriate function of coordinates for moment of inertia calculation

    // JR: CONSIDER VIRTUAL PARTICLES

    for (i = 0; i < nbins; i++) {
        n = 0;
        if (angularflag) {
            rsum[0] = rsum[1] = rsum[2] = 0.0;
            Isum[0] = Isum[1] = Isum[2] = Isum[3] = Isum[4] = Isum[5] = 0.0;
        }
        ...
        for (j = binhead[i]; j >= 0; j = binnext[j]) {
            ...
            if (angularflag) {
                ...
                if (dimension == 3) {
                    rsum[2] += x[j][2];
                    Isum[0] += x[j][1] * x[j][1] + x[j][2] * x[j][2];
                    Isum[1] += x[j][0] * x[j][0] + x[j][2] * x[j][2];
                    Isum[2] += x[j][0] * x[j][0] + x[j][1] * x[j][1];
                    Isum[3] -= x[j][0] * x[j][1];
                    Isum[4] -= x[j][0] * x[j][2];
                    Isum[5] -= x[j][1] * x[j][2];
                }
        ...
        if (angularflag) {
            vbin[i].rsum[0] = rsum[0];
            vbin[i].rsum[1] = rsum[1];
            if (dimension == 3) {
                vbin[i].rsum[2] = rsum[2];
                vbin[i].Isum[0] = Isum[0];
                vbin[i].Isum[1] = Isum[1];
                vbin[i].Isum[2] = Isum[2];
                vbin[i].Isum[3] = Isum[3];
                vbin[i].Isum[4] = Isum[4];
                vbin[i].Isum[5] = Isum[5];
            }
            else {
                vbin[i].Isum[0] = Isum[0];
                vbin[i].Isum[1] = Isum[1];
    ...
    if (shifts[1].commflag) vbin_comm(1);

    // for each bin I have particles contributing to:
    // compute vave and rcm over particles in bin

    double tfactor = force->mvv2e * mass_mpcd / (dimension * force->boltz);
    int dof_temp = 1;
    ...
    for (i = 0; i < nbins; i++) {
        ...
        vave[0] /= n;
        vave[1] /= n;
        if (dimension == 3) vave[2] /= n;
        if (angularflag) {
            rcm[0] /= n;
            rcm[1] /= n;
            if (dimension == 3) rcm[2] /= n;

            // Complete the calculation of the Moment of Inertia tensor
            if (dimension == 2) {
                vbin[i].Isum[0] = (vbin[i].Isum[0] - n * rcm[1] * rcm[1]);
                vbin[i].Isum[1] = (vbin[i].Isum[1] - n * rcm[0] * rcm[0]);
                vbin[i].Isum[2] = (vbin[i].Isum[2] - n * (rcm[0] * rcm[0] + rcm[1] * rcm[1]));
                vbin[i].Isum[3] = (vbin[i].Isum[3] + n * rcm[0] * rcm[1]);
            }
            else {
                vbin[i].Isum[0] = (vbin[i].Isum[0] - n * (rcm[1] * rcm[1] + rcm[2] * rcm[2]));
                vbin[i].Isum[1] = (vbin[i].Isum[1] - n * (rcm[0] * rcm[0] + rcm[2] * rcm[2]));
                vbin[i].Isum[2] = (vbin[i].Isum[2] - n * (rcm[0] * rcm[0] + rcm[1] * rcm[1]));
                vbin[i].Isum[3] = (vbin[i].Isum[3] + n * rcm[0] * rcm[1]);
                vbin[i].Isum[4] = (vbin[i].Isum[4] + n * rcm[0] * rcm[2]);
                vbin[i].Isum[5] = (vbin[i].Isum[5] + n * rcm[1] * rcm[2]);
            }
        }
        vsq = 0.0;
        vsum[0] = vsum[1] = vsum[2] = 0;
        DL[0] = DL[1] = DL[2] = 0;
        double factor = sqrt(force->boltz*temperature_mpcd / mass_mpcd / force->mvv2e);
        for (j = binhead[i]; j >= 0; j = binnext[j]) {
            // Generate Gaussian velocity
            vx = factor * random->gaussian();
            vy = factor * random->gaussian();
            if (dimension == 3)
                vz = factor * random->gaussian();
            // HERE IS WHERE WE CALCULATE THE ANGULAR MOMENTUM CHANGE DL
            if (angularflag) {
                if (dimension == 3) {
                    DL[0] += (x[j][1] - rcm[1])*(v[j][2] - vz) - (x[j][2] - rcm[2])*(v[j][1] - vy);
                    DL[1] += (x[j][2] - rcm[2])*(v[j][0] - vx) - (x[j][0] - rcm[0])*(v[j][2] - vz);
                    DL[2] += (x[j][0] - rcm[0])*(v[j][1] - vy) - (x[j][1] - rcm[1])*(v[j][0] - vx);
                }
                else {
                    DL[2] += (x[j][0] - rcm[0])*(v[j][1] - vy) - (x[j][1] - rcm[1])*(v[j][0] - vx);
                }
            }
            // Velocity of particle = vave + V_gaussian
            v[j][0] = vave[0] + vx;
            v[j][1] = vave[1] + vy; 
            if (dimension == 3)
                v[j][2] = vave[2] + vz;
            // Add Gaussian velocity to vsum
            vsum[0] += vx;
            vsum[1] += vy;
            if (dimension == 3)
                vsum[2] += vz;
        ...
        // Complete angular momentum conservation
        // Calculate wL=m*Isum^-1*DL
        if (angularflag) {
            if (dimension == 2) {
                Iinv[2] = 1.0 / vbin[i].Isum[2];
                wL[2] = Iinv[2] * DL[2];
            }
            else {
                // 0 Ixx, 1 Iyy, 2 Izz, 3 Ixy, 4 Ixz, 5 Iyz       
                double factI = 1.0 / (2 * vbin[i].Isum[4]* vbin[i].Isum[3]* vbin[i].Isum[5]
                    - vbin[i].Isum[4] * vbin[i].Isum[4] * vbin[i].Isum[1] 
                    - vbin[i].Isum[3]* vbin[i].Isum[3] * vbin[i].Isum[2] 
                    + vbin[i].Isum[0] * (vbin[i].Isum[1] * vbin[i].Isum[2] - vbin[i].Isum[5]* vbin[i].Isum[5]));
                Iinv[0] = factI * (vbin[i].Isum[1] * vbin[i].Isum[2] - vbin[i].Isum[5] * vbin[i].Isum[5]);
                Iinv[1] = factI * (vbin[i].Isum[0] * vbin[i].Isum[2] - vbin[i].Isum[4] * vbin[i].Isum[4]);
                Iinv[2] = factI * (vbin[i].Isum[0] * vbin[i].Isum[1] - vbin[i].Isum[3] * vbin[i].Isum[3]);
                Iinv[3] = factI * (vbin[i].Isum[4] * vbin[i].Isum[5] - vbin[i].Isum[3] * vbin[i].Isum[2]);
                Iinv[4] = factI * (vbin[i].Isum[3] * vbin[i].Isum[5] - vbin[i].Isum[4] * vbin[i].Isum[1]);
                Iinv[5] = factI * (vbin[i].Isum[3] * vbin[i].Isum[4] - vbin[i].Isum[0] * vbin[i].Isum[5]);
                wL[0] = Iinv[0] * DL[0] + Iinv[3] * DL[1] + Iinv[4] * DL[2];
                wL[1] = Iinv[3] * DL[0] + Iinv[1] * DL[1] + Iinv[5] * DL[2];
                wL[2] = Iinv[4] * DL[0] + Iinv[5] * DL[1] + Iinv[2] * DL[2];
            }

        }

        for (j = binhead[i]; j >= 0; j = binnext[j]) {
            // Velocity of particle - bin_total
            v[j][0] -= vsum[0];
            v[j][1] -= vsum[1];
            if (dimension == 3)
                v[j][2] -= vsum[2];
            ...
            if (angularflag) {
                // Add wL x (x[i][] - rcm[])
                if (dimension == 2) {
                    v[j][0] -= wL[2] * (x[j][1] - rcm[1]);
                    v[j][1] += wL[2] * (x[j][0] - rcm[0]);
                }
                else {
                    v[j][0] += wL[1] * (x[j][2] - rcm[2]) - wL[2] * (x[j][1] - rcm[1]);
                    v[j][1] += wL[2] * (x[j][0] - rcm[0]) - wL[0] * (x[j][2] - rcm[2]);
                    v[j][2] += wL[0] * (x[j][1] - rcm[1]) - wL[1] * (x[j][0] - rcm[0]);
                }
            }
        }
    ...
    if (shifts[1].commflag) xbin_comm(1, 1);

    for (i = 0; i < nbins; i++) {
        if (vbin[i].owner) {
            if (vbin[i].n > 1) {
                mpcd_bin_temp += vbin[i].value[0] / (vbin[i].n - dof_temp);
                mpcd_bin_count++;
            }
        }
    }

    mpcd_bin_temp *= tfactor;
}
...
void FixMPCD::vbin_comm(int ishift)
{
...
void FixMPCD::vbin_pack(BinAve *vbin, int n, int *list, double *buf)
{
    int j;
    int m = 0;
    for (int i = 0; i < n; i++) {
...
void FixMPCD::vbin_unpack(double *buf, BinAve *vbin, int n, int *list)
{
    int j;
    int m = 0;
    for (int i = 0; i < n; i++) {
...
void FixMPCD::xbin_comm(int ishift, int nval)
{
    BinComm *bcomm1, *bcomm2;
    MPI_Request request1, request2;
...
void FixMPCD::xbin_pack(BinAve *vbin, int n, int *list, double *buf, int nval)
{
    int j, k;
    int m = 0;
    for (int i = 0; i < n; i++) {
...
void FixMPCD::xbin_unpack(double *buf, BinAve *vbin, int n, int *list, int nval)
{
    int j, k;
    int m = 0;
    for (int i = 0; i < n; i++) {
...
/* ----------------------------------------------------------------------
   detect all collisions between MPCD and BIG particles or WALLS
   assume MPCD can be inside at most one BIG particle or WALL at a time
   unoverlap MPCDs for each collision
------------------------------------------------------------------------- */

void FixMPCD::collisions_single()
{
    ...
    // outer loop over MPCD particles
    // inner loop over BIG particles or WALLS that overlap MPCD particle bin
    // if overlap between MPCD and BIG particle or wall:
    //   for exact, compute collision pt in time
    //   for inexact, push MPCD to surf of BIG particle or WALL
    // update x,v of MPCD and f,torque on BIG particle
    // re-bin MPCD particle after collision
    // iterate until the MPCD particle has no overlaps with BIG particles or WALLS
                ...
                if (inside) {
...
#ifdef MPCD_DEBUG
                    if (update->ntimestep == MPCD_DEBUG_TIMESTEP &&
                        atom->tag[i] == MPCD_DEBUG_ATOMID)
                    ...
                    if (t_remain > dt) {
                        ...
                        if (insideflag == INSIDE_ERROR || insideflag == INSIDE_WARN) {
                            ...
                            if (type != WALL) {
                                sprintf(str,
                                    "MPCD particle " TAGINT_FORMAT " started "
                    ...
                    // update BIG particle and WALL and MPCD
                    ...
                    ibin = binmpcd[i] = update_mpcd(i, t_remain, xscoll, vsnew, x[i], v[i]);
...
/* ----------------------------------------------------------------------
   detect all collisions between MPCD and big particles
   an MPCD can be inside more than one big particle at a time
   requires finding which big particle MPCD collided with first
   unoverlap MPCDs for each collision
------------------------------------------------------------------------- */

void FixMPCD::collisions_multi()
{
    ...
    // outer loop over MPCD particles
    // inner loop over BIG particles or WALLS that overlap MPCD particle bin
    // loop over all BIG and WALLS to find which one MPCD collided with first
    // if overlap between MPCD and BIG particle or wall:
    ...
    for (i = 0; i < nlocal; i++) {
        ...
        while (1) {
            ...
            ibin = binmpcd[i] = update_mpcd(i, t_first, xscoll, vsnew, x[i], v[i]);
...
/* ----------------------------------------------------------------------
   check if MPCD particle S is inside spherical big particle B
------------------------------------------------------------------------- */

int FixMPCD::inside_sphere(double *xs, double *xb, Big *big)
{
    double dx, dy, dz;
...
/* ----------------------------------------------------------------------
   check if MPCD particle S is inside ellipsoidal big particle B
------------------------------------------------------------------------- */

int FixMPCD::inside_ellipsoid(double *xs, double *xb, Big *big)
{
    double x, y, z;
...
/* ----------------------------------------------------------------------
   check if MPCD particle S is inside line big particle B
...
int FixMPCD::inside_line(double *xs, double *xb, double *vs, double *vb,
...
/* ----------------------------------------------------------------------
   check if MPCD particle S is inside triangle big particle B
   collision only possible if:
...
int FixMPCD::inside_tri(double *xs, double *xb, double *vs, double *vb,
    Big *big, double dt_step)
{
...
/* ----------------------------------------------------------------------
   check if MPCD particle S is inside wall IWALL
------------------------------------------------------------------------- */

int FixMPCD::inside_wall(double *xs, int iwall)
{
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with surface of spherical big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of MPCD at time of collision
   xbcoll = position of big particle at time of collision
   norm = surface normal of collision pt at time of collision
------------------------------------------------------------------------- */

double FixMPCD::collision_sphere_exact(double *xs, double *xb,
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with surface of spherical big particle B
   inexact because just push MPCD to surface of big particle at end of step
...
void FixMPCD::collision_sphere_inexact(double *xs, double *xb,
    Big *big,
    double *xscoll, double *xbcoll,
    double *norm)
{
...
/* ----------------------------------------------------------------------
collision of MPCD particle S with surface of spherical big particle B
 1) advance particle to C dt/2 (can be inside or outside particle)
 2) put particle at point D (closest from C to sphere)
 3) Bounce back + velocity due to big particle
  v_i = -v_i + 2*(v_s + w_B x (D-r_B) + v_B)
  where:
    v_i : velocity of MPCD particle
    v_s : slip velocity of squirmer (0 for passive particles)
    w_B : angular velocity of big particle
    r_B : center of big particle
    v_B : velocity of big particle
...
void FixMPCD::collision_sphere_Padding(double *xs, double *xb,
    Big *big,
    double *xscoll, double *xbcoll,
    double *norm)
{
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with surface of ellipsoidal big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of MPCD at time of collision
   xbcoll = position of big particle at time of collision
...
double FixMPCD::collision_ellipsoid_exact(double *xs, double *xb,
    double *vs, double *vb, Big *big,
    double *xscoll, double *xbcoll,
    double *norm)
{
...
void FixMPCD::collision_ellipsoid_inexact(double *xs, double *xb,
    Big *big,
    double *xscoll, double *xbcoll,
    double *norm)
{
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with surface of line big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of MPCD at time of collision
   xbcoll = position of big particle at time of collision
...
double FixMPCD::collision_line_exact(double *xs, double *xb,
    double *vs, double *vb, Big *big,
    double dt_step,
    double *xscoll, double *xbcoll,
    double *norm)
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with surface of triangle big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of MPCD at time of collision
   xbcoll = position of big particle at time of collision
...
double FixMPCD::collision_tri_exact(double *xs, double *xb,
    double *vs, double *vb, Big *big,
    double dt_step,
    double *xscoll, double *xbcoll,
    double *norm)
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with wall IWALL
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of MPCD at time of collision
...
double FixMPCD::collision_wall_exact(double *xs, int iwall, double *vs,
    double *xscoll, double *xbcoll,
    double *norm)
{
    int dim = wallwhich[iwall] / 2;
...
/* ----------------------------------------------------------------------
   collision of MPCD particle S with wall IWALL
   inexact because just push MPCD to surface of wall at end of step
   time of collision = end of step
   xscoll = collision pt = position of MPCD at time of collision
   xbcoll = position of wall at time of collision
   norm = surface normal of collision pt at time of collision
...
void FixMPCD::collision_wall_inexact(double *xs, int iwall, double *xscoll,
...
/* ----------------------------------------------------------------------
   ...
   vs = velocity of MPCD, vb = velocity of BIG
   ...
   v of BIG particle in direction of surf normal is added to v of MPCD
   includes component due to rotation of BIG
   return vsnew of MPCD
------------------------------------------------------------------------- */

void FixMPCD::slip(double *vs, double *vb, double *xb, Big *big,
...
/* ----------------------------------------------------------------------
   ...
   v of WALL in direction of surf normal is added to v of MPCD
   return vsnew of MPCD
------------------------------------------------------------------------- */

void FixMPCD::slip_wall(double *vs, int iwall, double *norm, double *vsnew)
{
...
/* ----------------------------------------------------------------------
   ...
   xsurf = collision pt on surf of BIG
   norm = unit normal from surface of BIG at collision pt
   v of collision pt is added to v of MPCD
   includes component due to rotation of BIG
   return vsnew of MPCD
------------------------------------------------------------------------- */

void FixMPCD::noslip(double *vs, double *vb, double *xb, Big *big, int iwall,
    double *xsurf, double *norm, double *vsnew)
{
    double vs_dot_n, scale, r1, r2, vnmag, vtmag1, vtmag2;
    double tangent1[3], tangent2[3];
...
/* ----------------------------------------------------------------------
   ...
   force on BIG particle = -dp/dt of MPCD particle
   torque on BIG particle = r cross (-dp/dt)
------------------------------------------------------------------------- */

void FixMPCD::force_torque(double *vsold, double *vsnew,
    double *xs, double *xb,
    double *fb, double *tb)
{
    double dpdt[3], xs_xb[3];

    double factor = mass_mpcd / dt_big / force->ftm2v;
...
/* ----------------------------------------------------------------------
   ...
   force on WALL = -dp/dt of MPCD particle
------------------------------------------------------------------------- */

void FixMPCD::force_wall(double *vsold, double *vsnew, int iwall)
...
/* ----------------------------------------------------------------------
   update MPCD particle position & velocity & search bin assignment
   check if MPCD moved outside of valid region
   if so, may overlap off-processor BIG particle
------------------------------------------------------------------------- */

int FixMPCD::update_mpcd(int i, double dt, double *xscoll, double *vsnew,
    double *xs, double *vs)
{
    int ix, iy, iz;
    ...
    if (xs[0] < mpcdlo[0] || xs[0] > mpcdhi[0] ||
        xs[1] < mpcdlo[1] || xs[1] > mpcdhi[1] ||
        xs[2] < mpcdlo[2] || xs[2] > mpcdhi[2]) {
        if (screen) {
            error->warning(FLERR, "Fix mpcd particle moved outside valid domain");
            ...
            fprintf(screen, "  mpcdlo/hi x %g %g\n", mpcdlo[0], mpcdhi[0]);
            fprintf(screen, "  mpcdlo/hi y %g %g\n", mpcdlo[1], mpcdhi[1]);
            fprintf(screen, "  mpcdlo/hi z %g %g\n", mpcdlo[2], mpcdhi[2]);
        }
    }
...
/* ----------------------------------------------------------------------
   setup all MPCD parameters with big particles
------------------------------------------------------------------------- */

void FixMPCD::parameterize()
{
    // timesteps

    dt_big = update->dt;
    dt_mpcd = nevery * update->dt;
    ...
    for (int i = 0; i < nlocal; i++)
        if (mask[i] & biggroupbit) {
            if (radius && radius[i] > 0.0) {
                ...
                error->one(FLERR, "Big particle in fix mpcd cannot be point particle");
        }
    ...
    if (any_lines && overlap == 0)
        error->all(FLERR, "Cannot use lines with fix mpcd unless overlap is set");
    if (any_tris && overlap == 0)
        error->all(FLERR, "Cannot use tris with fix mpcd unless overlap is set");

    // big particles are only torqued if ellipsoids/lines/tris or NOSLIP

    torqueflag = 1;

    // mass of MPCD particles, require monodispersity
    ...
    for (int i = 0; i < nlocal; i++)
        if (mask[i] & groupbit) {
            if (rmass) {
                if (mass_mpcd == 0.0) mass_mpcd = rmass[i];
                else if (rmass[i] != mass_mpcd) flag = 1;
            }
            else {
                if (mass_mpcd == 0.0) mass_mpcd = mass[type[i]];
                else if (mass[type[i]] != mass_mpcd) flag = 1;
            }
        }
    ...
    if (flagall)
        error->all(FLERR, "Fix mpcd requires MPCD particles all have same mass");

    // set temperature and lamda of MPCD particles from each other
    // lamda = dt_mpcd * sqrt(boltz * temperature_mpcd / mass_mpcd);

    lamda = dt_mpcd * sqrt(force->boltz*temperature_mpcd / mass_mpcd / force->mvv2e);

    // vmax = maximum velocity of an MPCD particle
    // dmax = maximum distance an MPCD can move = 4*lamda = vmax * dt_mpcd
    ...
    bigint nmpcd = group->count(igroup);
    bigint nvirtual = 0;
    if (virtualexist) nvirtual = group->count(virtualgroup);
    bigint expectedvirtual = 0;
    if (dimension == 2) {
        expectedvirtual = (int) round(M_PI*shell*mbig*(maxbigdiam - shell)*gamma);
    }
    else if (dimension == 3) {
        expectedvirtual = (int)round(M_PI*shell*mbig*(maxbigdiam - shell)*gamma);
    }
    ...
    // mass density ratio = big / MPCD

    double density_big = 0.0;
    if (bigexist) density_big = mass_big / volbig;

    double volmpcd, density_mpcd;
    if (dimension == 3) {
        volmpcd = (domain->xprd * domain->yprd * domain->zprd) - volbig;
        density_mpcd = nmpcd * mass_mpcd /
            (domain->xprd*domain->yprd*domain->zprd - volbig);
    }
    else {
        volmpcd = (domain->xprd * domain->yprd) - volbig;
        density_mpcd = nmpcd * mass_mpcd /
            (domain->xprd*domain->yprd - volbig);
    }

    double mdratio = density_big / density_mpcd;

    // create grid for binning/rotating MPCD particles from gridmpcd
    ...
    // mpcd_per_grid = # of MPCD particles per MPCD grid cell

    double ncell;
    if (dimension == 3)
        ncell = volmpcd / (binsize3x*binsize3y*binsize3z);
    else
        ncell = volmpcd / (binsize3x*binsize3y);

    mpcd_per_cell = (double)nmpcd / ncell;

    // kinematic viscosity of MPCD fluid
    // output in cm^2/sec units, converted by xxt2kmu

    double viscosity;
    if (dimension == 3)
        viscosity = gridmpcd * gridmpcd / (18.0*dt_mpcd) *
        (1.0 - (1.0 - exp(-mpcd_per_cell)) / mpcd_per_cell) +
        (force->boltz*temperature_mpcd*dt_mpcd / (4.0*mass_mpcd*force->mvv2e)) *
        ((mpcd_per_cell + 2.0) / (mpcd_per_cell - 1.0));
    else
        viscosity =
        (force->boltz*temperature_mpcd*dt_mpcd / (2.0*mass_mpcd*force->mvv2e)) *
        (mpcd_per_cell / (mpcd_per_cell - 1.0 + exp(-mpcd_per_cell)) - 1.0) +
        (gridmpcd*gridmpcd) / (12.0*dt_mpcd) *
        ((mpcd_per_cell - 1.0 + exp(-mpcd_per_cell)) / mpcd_per_cell);
    viscosity *= force->xxt2kmu;

    // print MPCD parameters

    if (me == 0) {
        if (screen) {
            fprintf(screen, "MPCD info:\n");
            fprintf(screen,
                "  MPCD/big/virtual/expected_virtual particles = " BIGINT_FORMAT " " BIGINT_FORMAT " " BIGINT_FORMAT " " BIGINT_FORMAT "\n",
                nmpcd, mbig, nvirtual, expectedvirtual);
            fprintf(screen, "  big particle diameter max/min = %g %g\n",
                maxbigdiam, minbigdiam);
            fprintf(screen, "  MPCD temperature & lamda = %g %g\n",
                temperature_mpcd, lamda);
            fprintf(screen, "  MPCD max distance & max velocity = %g %g\n", dmax, vmax);
            fprintf(screen, "  MPCD grid counts: %d %d %d\n", nbin1x, nbin1y, nbin1z);
            fprintf(screen, "  MPCD grid size: request, actual (xyz) = %g, %g %g %g\n",
                gridmpcd, binsize3x, binsize3y, binsize3z);
            fprintf(screen, "  MPCD per actual grid cell = %g\n", mpcd_per_cell);
            fprintf(screen, "  MPCD viscosity = %g\n", viscosity);
            fprintf(screen, "  big/MPCD mass density ratio = %g\n", mdratio);
        }
        if (logfile) {
            fprintf(logfile, "MPCD info:\n");
            fprintf(logfile,
                "  MPCD/big particles = " BIGINT_FORMAT " " BIGINT_FORMAT " " BIGINT_FORMAT "\n",
                nmpcd, mbig, nvirtual);
            fprintf(logfile, "  big particle diameter max/min = %g %g\n",
                maxbigdiam, minbigdiam);
            fprintf(logfile, "  MPCD temperature & lamda = %g %g\n",
                temperature_mpcd, lamda);
            fprintf(logfile, "  MPCD max distance & max velocity = %g %g\n", dmax, vmax);
            fprintf(logfile, "  MPCD grid counts: %d %d %d\n", nbin1x, nbin1y, nbin1z);
            fprintf(logfile, "  MPCD grid size: request, actual (xyz) = %g, %g %g %g\n",
                gridmpcd, binsize3x, binsize3y, binsize3z);
            fprintf(logfile, "  MPCD per actual grid cell = %g\n", mpcd_per_cell);
            fprintf(logfile, "  MPCD viscosity = %g\n", viscosity);
            fprintf(logfile, "  big/MPCD mass density ratio = %g\n", mdratio);
        }
    }

    // error if less than 1 MPCD bin per processor in some dim

    if (nbin1x < comm->procgrid[0] || nbin1y < comm->procgrid[1] ||
        nbin1z < comm->procgrid[2])
        error->all(FLERR, "Fewer MPCD bins than processors in some dimension");


    // error if lamda < 0.6 of MPCD grid size and no shifting allowed
    // turn on shifting in this case if allowed

    double maxgridmpcd = MAX(binsize3x, binsize3y);
    if (dimension == 3) maxgridmpcd = MAX(maxgridmpcd, binsize3z);

    // warnings

    if (bigexist && maxgridmpcd > 0.25 * minbigdiam && me == 0)
        error->warning(FLERR, "Fix mpcd grid size > 1/4 of big particle diameter");
    if (viscosity < 0.0 && me == 0)
        error->warning(FLERR, "Fix mpcd viscosity < 0.0 due to low MPCD density");
    if (bigexist && dt_big*vmax > minbigdiam && me == 0)
        error->warning(FLERR, "Fix mpcd particles may move > big particle diameter");
}
...
/* ----------------------------------------------------------------------
   set bounds for big and MPCD particle movement
   called at setup() and when box size changes (but not shape)
------------------------------------------------------------------------- */

void FixMPCD::setup_bounds()
{
    ...
    // dist_mpcd = distance from SD at which MPCD could collide with unknown CO,
    //   used to error check bounds of MPCD movement after collisions via mpcdlo/hi
    // dist_mpcd_reneigh = distance from SD at which an MPCD should trigger
    //   a reneigh, b/c next MPCD move might overlap with unknown CO,
    //   used for MPCD triggering of reneighboring via mpcdlo/hi_reneigh
    // onemove = max distance an MPCD can move in one step
    // if big_particles (and possibly walls):
    //   dist_ghost = cut + 1/2 skin due to moving away before reneigh
    //   dist_mpcd = cut - 1/2 skin - 1/2 diam due to ghost CO moving towards
    //   dist_reneigh = dist_mpcd - onemove
    // if walls and no big particles:
    //   dist_ghost = 0.0, since not used
    // if no big particles or walls:
    //   dist_ghost and dist_mpcd = 0.0, since not used since no search bins
    //   dist_mpcd_reneigh = subsize - onemove =
    ...
    if (bigexist) {
        ...
        dist_mpcd = cut - 0.5*neighbor->skin - 0.5*maxbigdiam;
        dist_mpcd_reneigh = dist_mpcd - onemove;
    }
    else if (wallexist) {
        dist_ghost = 4 * onemove;
        dist_mpcd = 4 * onemove;
        dist_mpcd_reneigh = 4 * onemove - onemove;
    }
    else {
        dist_ghost = dist_mpcd = 0.0;
        double subsize;
        if (triclinic == 0) {
    ...
    // lo/hi = bbox on this proc which MPCD particles must stay inside
    // lo/hi reneigh = bbox on this proc outside of which MPCDs trigger a reneigh
    // for triclinic, these bbox are in lamda units

    if (triclinic == 0) {
        mpcdlo[0] = domain->sublo[0] - dist_mpcd;
        mpcdhi[0] = domain->subhi[0] + dist_mpcd;
        mpcdlo[1] = domain->sublo[1] - dist_mpcd;
        mpcdhi[1] = domain->subhi[1] + dist_mpcd;
        mpcdlo[2] = domain->sublo[2] - dist_mpcd;
        mpcdhi[2] = domain->subhi[2] + dist_mpcd;

        mpcdlo_reneigh[0] = domain->sublo[0] - dist_mpcd_reneigh;
        mpcdhi_reneigh[0] = domain->subhi[0] + dist_mpcd_reneigh;
        mpcdlo_reneigh[1] = domain->sublo[1] - dist_mpcd_reneigh;
        mpcdhi_reneigh[1] = domain->subhi[1] + dist_mpcd_reneigh;
        mpcdlo_reneigh[2] = domain->sublo[2] - dist_mpcd_reneigh;
        mpcdhi_reneigh[2] = domain->subhi[2] + dist_mpcd_reneigh;

    }
    else {
        mpcdlo[0] = domain->sublo_lamda[0] - dist_mpcd * length0;
        mpcdhi[0] = domain->subhi_lamda[0] + dist_mpcd * length0;
        mpcdlo[1] = domain->sublo_lamda[1] - dist_mpcd * length1;
        mpcdhi[1] = domain->subhi_lamda[1] + dist_mpcd * length1;
        mpcdlo[2] = domain->sublo_lamda[2] - dist_mpcd * length2;
        mpcdhi[2] = domain->subhi_lamda[2] + dist_mpcd * length2;

        mpcdlo_reneigh[0] = domain->sublo_lamda[0] - dist_mpcd_reneigh * length0;
        mpcdhi_reneigh[0] = domain->subhi_lamda[0] + dist_mpcd_reneigh * length0;
        mpcdlo_reneigh[1] = domain->sublo_lamda[1] - dist_mpcd_reneigh * length1;
        mpcdhi_reneigh[1] = domain->subhi_lamda[1] + dist_mpcd_reneigh * length1;
        mpcdlo_reneigh[2] = domain->sublo_lamda[2] - dist_mpcd_reneigh * length2;
        mpcdhi_reneigh[2] = domain->subhi_lamda[2] + dist_mpcd_reneigh * length2;
    }
}

/* ----------------------------------------------------------------------
   setup bins used for binning MPCD particles for velocity reset
   gridmpcd = desired bin size
...
void FixMPCD::setup_velocity_bins()
{
    // require integer # of bins across global domain

    nbin1x = static_cast<int> (domain->xprd / gridmpcd + 0.5);
    nbin1y = static_cast<int> (domain->yprd / gridmpcd + 0.5);
    nbin1z = static_cast<int> (domain->zprd / gridmpcd + 0.5);
    ...
    if (max > maxbin1) {
        memory->destroy(binhead);
        maxbin1 = max;
        memory->create(binhead, max, "fix/mpcd:binhead");
        memory->destroy(binheadv);
        memory->create(binheadv, max, "fix/mpcd:binheadv");
    }
    ...
    if (max > maxbuf) {
        ...
        memory->destroy(rbuf2);
        maxbuf = max;
        memory->create(sbuf1, max*VBINSIZE, "fix/mpcd:sbuf");
        memory->create(sbuf2, max*VBINSIZE, "fix/mpcd:sbuf");
        memory->create(rbuf1, max*VBINSIZE, "fix/mpcd:rbuf");
        memory->create(rbuf2, max*VBINSIZE, "fix/mpcd:rbuf");
    }
...
void FixMPCD::setup_velocity_shift(int ishift, int dynamic)
{
    ...
    if (reallocflag) {
        ...
        memory->create(first->sendlist, nbinsq, "fix/mpcd:sendlist");
        memory->create(first->recvlist, nbinsq, "fix/mpcd:sendlist");
        memory->create(second->sendlist, nbinsq, "fix/mpcd:sendlist");
        memory->create(second->recvlist, nbinsq, "fix/mpcd:sendlist");
    }
    ...
    if (dimension == 3) {
        ...
        if (reallocflag) {
            ...
            memory->destroy(second->sendlist);
            memory->destroy(second->recvlist);
            memory->create(first->sendlist, nbinx*nbiny, "fix/mpcd:sendlist");
            memory->create(first->recvlist, nbinx*nbiny, "fix/mpcd:sendlist");
            memory->create(second->sendlist, nbinx*nbiny, "fix/mpcd:sendlist");
            memory->create(second->recvlist, nbinx*nbiny, "fix/mpcd:sendlist");
        }
    ...
    if (dynamic == 0 && nbins > shifts[ishift].maxvbin) {
        ...
        shifts[ishift].vbin = (BinAve *)
            memory->smalloc(nbins * sizeof(BinAve), "fix/mpcd:vbin");
    }
...
/* ----------------------------------------------------------------------
   setup bins used for big and MPCD particle searching
...
void FixMPCD::setup_search_bins()
{
    ...
    if (nbins2 > maxbin2) {
        ...
        memory->destroy(binbig);
        maxbin2 = nbins2;
        memory->create(nbinbig, nbins2, "fix/mpcd:nbinbig");
        memory->create(binbig, nbins2, ATOMPERBIN, "fix/mpcd:binbig");
    }
}
...
void FixMPCD::setup_search_stencil()
{
    ...
    if (max > maxstencil) {
        memory->destroy(stencil);
        maxstencil = max;
        memory->create(stencil, max, 4, "fix/mpcd:stencil");
    }
...
double FixMPCD::point_bin_distance(double *x, int i, int j, int k)
{
...
double FixMPCD::bin_bin_distance(int i, int j, int k)
{
    double delx, dely, delz;
...
int FixMPCD::pack_reverse_comm(int n, int first, double *buf)
{
    int i, m, last;
...
void FixMPCD::unpack_reverse_comm(int n, int *list, double *buf)
{
    int i, j, m;
...
double FixMPCD::compute_vector(int n)
{
    ...
    if (stats_flag == 0) {
        ...
        stats[5] = nbins2;
        stats[6] = nbins1;
        stats[7] = mpcd_bin_count;
        stats[8] = mpcd_bin_temp;
...
void FixMPCD::velocity_stats(int groupnum)
{
...
double FixMPCD::newton_raphson(double t1, double t2)
{
...
void FixMPCD::lineside(double t, double &f, double &df)
{
    double p[2], c[2];
...
void FixMPCD::triside(double t, double &f, double &df)
{
    double p[2], c[2];
...
double FixMPCD::distance(int i, int j)
{
...
void FixMPCD::print_collision(int i, int j, int ibounce,
    ...
    int type)
{
    ...
    if (type != WALL) {
        printf("COLLISION between MPCD " TAGINT_FORMAT
        ...
        printf("COLLISION between MPCD " TAGINT_FORMAT " and WALL %d\n",
            atom->tag[i], j);
        printf("  bounce # = %d\n", ibounce + 1);
        printf("  local indices: %d %d\n", i, j);
        printf("  timestep = %g\n", dt);
