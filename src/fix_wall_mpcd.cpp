...
FixWallMPCD::FixWallMPCD(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg), nwall(0), fwall(NULL), fwall_all(NULL)
{
  if (narg < 4) error->all(FLERR,"Illegal fix wall/mpcd command");
  ...
  while (iarg < narg) {
    if ((strcmp(arg[iarg],"xlo") == 0) || (strcmp(arg[iarg],"xhi") == 0) ||
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix wall/mpcd command");
      ...
      for (int m = 0; (m < nwall) && (m < 6); m++)
        if (newwall == wallwhich[m])
          error->all(FLERR,"Wall defined twice in fix wall/mpcd command");
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal wall/mpcd command");
      if (strcmp(arg[iarg+1],"box") == 0) scaleflag = 0;
      else if (strcmp(arg[iarg+1],"lattice") == 0) scaleflag = 1;
      else error->all(FLERR,"Illegal fix wall/mpcd command");
      iarg += 2;
    } else error->all(FLERR,"Illegal fix wall/mpcd command");
  }
  ...
  for (int m = 0; m < nwall; m++) {
    if ((wallwhich[m] == XLO || wallwhich[m] == XHI) && domain->xperiodic)
      error->all(FLERR,"Cannot use fix wall/mpcd in periodic dimension");
  ...
  for (int m = 0; m < nwall; m++)
    if ((wallwhich[m] == ZLO || wallwhich[m] == ZHI) && domain->dimension == 2)
      error->all(FLERR,"Cannot use fix wall/mpcd zlo/zhi for a 2d simulation");
  ...
  memory->create(fwall,nwall,3,"wall/mpcd:fwall");
  memory->create(fwall_all,nwall,3,"wall/mpcd:fwall_all");
...
void FixWallMPCD::init()
{
  int flag = 0;
  for (int m = 0; m < modify->nfix; m++)
    if (strcmp(modify->fix[m]->style,"mpcd") == 0) flag = 1;
  if (!flag) error->all(FLERR,"Cannot use fix wall/mpcd without fix mpcd");

  for (int m = 0; m < nwall; m++) {
    if (wallstyle[m] != VARIABLE) continue;
    varindex[m] = input->variable->find(varstr[m]);
    if (varindex[m] < 0)
      error->all(FLERR,"Variable name for fix wall/mpcd does not exist");
    if (!input->variable->equalstyle(varindex[m]))
      error->all(FLERR,"Variable for fix wall/mpcd is invalid style");
  }
...
double FixWallMPCD::compute_array(int i, int j)
{
...
void FixWallMPCD::wall_params(int flag)
{
  double xnew;
