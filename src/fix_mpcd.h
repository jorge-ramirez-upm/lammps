...
FixStyle(mpcd,FixMPCD)

#else

#ifndef LMP_FIX_MPCD_H
#define LMP_FIX_MPCD_H

#include "fix.h"

namespace LAMMPS_NS {

class FixMPCD : public Fix {
 public:
  FixMPCD(class LAMMPS *, int, char **);
  ~FixMPCD();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  ...
  int virtualexist, virtualgroup, virtualgroupbit;
  double shell, gamma; // shell for virtual particles, # MPCD per grid 
  int collidestyle,overlap,insideflag,exactflag,angularflag,maxbounceallow;
  int cubicflag,shiftuser,shiftseed;
  double gridmpcd,gridsearch,lamda,radfactor,cubictol;
  int triclinic,change_size,change_shape,deformflag;

  double dt_big,dt_mpcd;
  double mass_big,mass_mpcd;
  double temperature_mpcd;
  double sigma;
  double mpcd_per_cell;
  double dmax,vmax,vmaxsq;
  double maxbigdiam,minbigdiam;
  double dist_ghost,dist_mpcd,dist_mpcd_reneigh;   // explained in code

  int wallexist,nwall,wallvarflag;
  class FixWallMPCD *wallfix;
  int *wallwhich;
  ...
  double mpcdlo[3],mpcdhi[3];                  // MPCDs must stay inside
  double mpcdlo_reneigh[3],mpcdhi_reneigh[3];  // MPCDs trigger a reneigh
  ...
  int nmax;               // max number of MPCD particles

  // bins for MPCD velocity remap, shifting and communication
  ...
  struct BinAve {
    ...
    int n;               // # of MPCD particles in bin
    double xctr[3];      // center point of bin, only used for triclinic
    double vsum[3];      // sum of v components for MPCD particles in bin
    double rsum[3];      // sum of r components for MPCD particles in bin (for angular momentum conservation)
    double value[12];    // extra per-bin values
    double Isum[6];         // Moment if Inertia tensor: Ixx, Iyy, Izz, Ixy, Ixz, Iyz
  };
  ...
  int maxbin1;
  int *binhead;          // 1st MPCD particle in each bin
  int *binheadv;          // 1st virtual MPCD particle in each bin
  int *binnext;          // next MPCD particle in same bin
  int *binnextv;          // next virtual MPCD particle in same bin
  int maxbuf;
  double *sbuf1,*sbuf2;  // buffers for send/recv of velocity bin data
  double *rbuf1,*rbuf2;
  int maxvirtual;    // Maximum number of virtual particles in the system
  double **xv, **vv; // Position an velocity of virtual particles

  // bins and stencil for collision searching for MPCDs & big particles
  ...
  int *binmpcd;           // which bin each MPCD particle is in
  int *binmpcdv;           // which bin each virtual MPCD particle is in
  ...
  void collision_sphere_inexact(double *, double *,
                                Big *, double *, double *, double *);
  void collision_sphere_Padding(double *, double *,
                                Big *, double *, double *, double *);
  ...
  int update_mpcd(int, double, double *, double *, double *, double *);
...
E: Could not find fix mpcd group ID

Self-explanatory.

E: Fix mpcd requires newton pair on

Self-explanatory.

E: Fix mpcd requires ghost atoms store velocity

Use the comm_modify vel yes command to enable this.

E: Fix mpcd no-slip requires atom attribute torque

This is because the MPCD collisions will impart torque to the solute
particles.

E: Cannot change timestep once fix mpcd is setup

This is because various MPCD properties depend on the timestep
size.

E: Fix mpcd can only currently be used with comm_style brick

This is a current restriction in LAMMPS.

E: Cannot use fix wall/mpcd more than once

Nor is their a need to since multiple walls can be specified
in one command.

W: Fix MPCD walls overlap but fix mpcd overlap not set

You likely want to set this in your input script.

E: Using fix mpcd with inconsistent fix deform remap option

When shearing the box in an MPCD simulation, the remap v option for fix
deform needs to be used.

W: Using fix mpcd with box deformation but no MPCD thermostat

The deformation will heat the MPCD particles so this can
be dangerous.

W: Fix mpcd MPCD moves may trigger frequent reneighboring

This is because the MPCD particles may move long distances.

E: Fix MPCD: bad search bin assignment

Something has gone wrong in your MPCD model; try using more
conservative settings.

E: Fix MPCD: bad stencil bin for big particle

Something has gone wrong in your MPCD model; try using more
conservative settings.

E: Fix MPCD: too many big particles in bin

Reset the ATOMPERBIN parameter at the top of fix_mpcd.cpp
to a larger value, and re-compile the code.

E: Fix MPCD: too many walls in bin

This should not happen unless your system has been setup incorrectly.

E: Fix MPCD: bad bin assignment for MPCD advection

Something has gone wrong in your MPCD model; try using more
conservative settings.

E: MPCD particle %d started inside big particle %d on step %ld bounce %d

See the inside keyword if you want this message to be an error vs
warning.

W: MPCD particle %d started inside big particle %d on step %ld bounce %d

See the inside keyword if you want this message to be an error vs
warning.

E: MPCD particle %d started inside wall %d on step %ld bounce %d

See the inside keyword if you want this message to be an error vs
warning.

W: MPCD particle %d started inside wall %d on step %ld bounce %d
...
W: Fix mpcd particle moved outside valid domain

This may indicate a problem with your simulation parameters.

E: Big particle in fix mpcd cannot be point particle

Big particles must be extended spheriods or ellipsoids.

E: Cannot use lines with fix mpcd unless overlap is set

This is because line segments are connected to each other.

E: Cannot use tris with fix mpcd unless overlap is set

This is because triangles are connected to each other.

E: Fix mpcd requires MPCD particles all have same mass

Self-explanatory.

E: Fewer MPCD bins than processors in some dimension

This is not allowed.  Make your MPCD bin size smaller.

E: MPCD bins for fix mpcd are not cubic enough

The bin shape is not within tolerance of cubic.  See the cubic
keyword if you want this message to be an error vs warning.

W: MPCD bins for fix mpcd are not cubic enough

The bin shape is not within tolerance of cubic.  See the cubic
keyword if you want this message to be an error vs warning.

E: MPCD bin size for fix mpcd differs from user request

Fix MPCD had to adjust the bin size to fit the simulation box.  See the
cubic keyword if you want this message to be an error vs warning.

W: MPCD bin size for fix mpcd differs from user request

Fix MPCD had to adjust the bin size to fit the simulation box.  See the
cubic keyword if you want this message to be an error vs warning.

E: Fix mpcd lamda must be >= 0.6 of MPCD grid size

This is a requirement for accuracy reasons.

W: MPCD bin shifting turned on due to small lamda

This is done to try to preserve accuracy.

W: Fix mpcd grid size > 1/4 of big particle diameter

This may cause accuracy problems.

W: Fix mpcd viscosity < 0.0 due to low MPCD density

This may cause accuracy problems.

W: Fix mpcd particles may move > big particle diameter
