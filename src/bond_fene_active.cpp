...
BondFENEActive::BondFENEActive(LAMMPS *lmp) : Bond(lmp)
{
...
void BondFENEActive::compute(int eflag, int vflag)
{
  int i1,i2,n,type;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    double faux=1.0;
    if (tag[i1]>tag[i2]) faux*=-1.0;
    if (newton_bond || i1 < nlocal) {
      f[i1][0] += delx*(fbond-faux*fact[type]);
      f[i1][1] += dely*(fbond-faux*fact[type]);
      f[i1][2] += delz*(fbond-faux*fact[type]);
    }

    if (newton_bond || i2 < nlocal) {
      f[i2][0] -= delx*(fbond+faux*fact[type]);
      f[i2][1] -= dely*(fbond+faux*fact[type]);
      f[i2][2] -= delz*(fbond+faux*fact[type]);
    }
...
void BondFENEActive::allocate()
{
  ...
  memory->create(fact,n+1,"bond::fact");
...
void BondFENEActive::coeff(int narg, char **arg)
{
  ...
  double fact_one = force->numeric(FLERR,arg[5]);
...
void BondFENEActive::init_style()
{
...
double BondFENEActive::equilibrium_distance(int i)
{
  return 0.97*sigma[i];
}
...
void BondFENEActive::read_restart(FILE *fp)
{
  allocate();
  ...
  if (comm->me == 0) {
    ...
    utils::sfread(FLERR,&fact[1],sizeof(double),atom->nbondtypes,fp,NULL,error);
  }
  ...
  MPI_Bcast(&fact[1],atom->nbondtypes,MPI_DOUBLE,0,world);
...
void BondFENEActive::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nbondtypes; i++)
    fprintf(fp,"%d %g %g %g %g %g\n",i,k[i],r0[i],epsilon[i],sigma[i],fact[i]);
}

/* ---------------------------------------------------------------------- */

double BondFENEActive::single(int type, double rsq, int /*i*/, int /*j*/,
                        double &fforce)
{
