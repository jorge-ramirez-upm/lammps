...
void PairPocillos::compute(int eflag, int vflag)
{
  ...
  double rsq,r,r2inv,rinv,forceij,factor_ij,tanhij;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        r = sqrt(rsq);
        rinv = 1.0 / r;
        tanhij = tanh((r - z0[itype][jtype])/dwell);
        forceij = -0.5*epsilon[itype][jtype]*(1 - tanhij*tanhij)*rinv/dwell;
...
void PairPocillos::coeff(int narg, char **arg)
{
  ...
  dwell = force->numeric(FLERR, arg[4]);
...
void PairPocillos::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g\n",i,j,z0[i][j],epsilon[i][j],cut[i][j]);
}

/* ---------------------------------------------------------------------- */

double PairPocillos::single(int i, int j, int itype, int jtype, double rsq,
