...
FixStyle(bond/create/destroy,FixBondCreateDestroy)

#else

#ifndef LMP_FIX_BOND_CREATE_DESTROY_H
#define LMP_FIX_BOND_CREATE_DESTROY_H

#include "fix.h"

namespace LAMMPS_NS {

class FixBondCreateDestroy : public Fix {
 public:
  FixBondCreateDestroy(class LAMMPS *, int, char **);
  ~FixBondCreateDestroy();
  int setmask();
  void init();
 ...
 private:
  ...
  double cutsq,cutminsq,pon,poff;
  int overflow;
  tagint lastcheck;
